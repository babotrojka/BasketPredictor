...
def read_extract_write(source_file: str, dest_file: str, class_to_extract):
    ...
    if class_to_extract in classes:
        ...
        result = {"visible": 1, "bbox": json_content["boxes"][0][index]}
    else:
        result = {"visible": 0, "bbox": np.zeros(4).tolist()}
...
def main(args):
    ...
    if not os.path.exists(json_folder) or not os.path.exists(images_folder):
        print(
            "Json folder or images folder does not exist."
            " Please provide valid folder"
        )
        sys.exit(0)
    ...
    for i, video_folder in enumerate(os.scandir(json_folder)):
        ...
        for json_txt_file in os.scandir(video_folder):
            dest_file = os.path.join(
                dest_folder, f"{os.path.splitext(json_txt_file.name)[0]}.json"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--images",
        "-i",
        help="Images parent folder. Structure must be the same as json folder",
        required=True,
    )
    parser.add_argument(
        "--e_class", "-c", help="Class you want to extract", type=int, default=32
    )
